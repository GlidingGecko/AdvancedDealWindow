...
local ms_HistoryListIM		:table		= InstanceManager:new( "HistoryList",  "List", Controls.HistoryListContainer );
...
local ms_DealHistory = {};
local ms_HistoryVisible = true;
...
-- ===========================================================================
function CreateHistoryGroup(rootStack : table, title : string)

	local iconList = ms_HistoryListIM:GetInstance(rootStack);
	if (title == nil or title == "") then
		iconList.Title:SetHide(true);		-- No title
	else
		iconList.TitleText:LocalizeAndSetText(title);
...
-- ===========================================================================
function SetHistoryValueText(icon, pDealItem)

	if (icon.ValueText ~= nil) then
		local valueName = pDealItem.ValueTypeNameID;
		if (valueName == nil) then
			--if (pDealItem:HasPossibleValues()) then
			--	valueName = "LOC_DIPLOMACY_DEAL_CLICK_TO_CHANGE_DEAL_PARAMETER";
			--end
		...
		if (ms_LastIncomingDealProposalAction == DealProposalAction.ACCEPTED or ms_LastIncomingDealProposalAction == DealProposalAction.PROPOSED or ms_LastIncomingDealProposalAction == DealProposalAction.ADJUSTED) then
			...
			if (DealManager.AreWorkingDealsEqual(g_LocalPlayer:GetID(), g_OtherPlayer:GetID())) then
				...
				local sendDealAndContinue = function() 
					...
					-- final accept, so store in history?
					local acceptedDeal = DealManager.GetWorkingDeal(DealDirection.OUTGOING, g_LocalPlayer:GetID(), g_OtherPlayer:GetID());
					StoreDealInHistory(g_OtherPlayer, acceptedDeal);
...
function PopulateAvailableResources(player : table, iconList : table, className : string)
	...
	local otherPlayerPossibleResources = DealManager.GetPossibleDealItems( GetOtherPlayer(player):GetID(), player:GetID(), DealItemTypes.RESOURCES);
	...
	local otherLeaderName = PlayerConfigurations[GetOtherPlayer(player):GetID()]:GetLeaderName();
		...
		for i, entry in ipairs(possibleResources) do
			...
			if (resourceDesc ~= nil) then
				...
				if (entry.MaxAmount > 0 and resourceDesc.ResourceClassType == className ) then
					...
					if MatchesPartnerResource(otherPlayerPossibleResources, resourceDesc.ResourceType) > -1 or MatchesPartnerResource(otherPlayerImportedResources, resourceDesc.ResourceType) > -1 then

						local duplicateText = Locale.Lookup("LOC_BDW_PLAYER_HAS_RESOURCE");

						if player:GetID() == g_LocalPlayer:GetID() then
							duplicateText = Locale.Lookup(otherLeaderName) .. " " .. Locale.Lookup("LOC_BDW_OTHER_HAS_RESOURCE");
						end

						icon = RenderResourceButton(player, entry, 'default', iconList, "[COLOR:GoldMetalDark]" .. duplicateText .. "[ENDCOLOR]");

						icon.AmountText:SetColor(UI.GetColorValue(255/255,223/255,0/255,230/255));

					else
						icon = RenderResourceButton(player, entry, 'default', iconList, '');
					end
	...
	if(playerImportedResources ~= nil) then
		...
		table.sort(playerImportedResources, sort_func);
		...
		for y, entry in ipairs(playerImportedResources) do
			if (entry.ClassType == className) then
				...
				icon = RenderResourceButton(player, entry, 'imported', iconList, entry.ImportString);
				icon.SelectButton:RegisterCallback( Mouse.eLClick, function() OnClickAvailableResource(player, entry.ForType); end );
	...
	-- Hide if empty
	iconList.GetTopControl():SetHide( iconList.ListStack:GetSizeY()==0 );
...
-- ===========================================================================
function PopulateDealBasic(player : table, iconList : table, populateType : number, iconName : string, pDeal)

	if(pDeal == nil) then
		pDeal = DealManager.GetWorkingDeal(DealDirection.OUTGOING, g_LocalPlayer:GetID(), g_OtherPlayer:GetID());
	end
			...
			if (pDealItem:GetFromPlayerID() == player:GetID()) then
				...
				if (type == DealItemTypes.GOLD) then
					...
					if (iDuration == 0) then
						...
						-- Multi-turn
						uiIcon = g_IconAndTextIM:GetInstance(iconList.For30TurnsDealsStack);
						uiIcon.IconText:LocalizeAndSetText("LOC_DIPLOMACY_DEAL_FOR_TURNS", iDuration);
						uiIcon.ValueText:SetHide(true);
					end
					SetIconToSize(uiIcon.Icon, "ICON_YIELD_GOLD_5");
...
-- ===========================================================================
function PopulatePlayerHistoryPanel(player : table)

	ms_HistoryListIM:ReleaseInstanceByParent(Controls.OfferHistoryStack);

	Controls.HistoryArea:SetHide(true);

	Controls.MinimiseHistory:RegisterCallback(Mouse.eLClick, function()
		ms_HistoryVisible = not ms_HistoryVisible;
		Controls.HistoryGrid:SetHide(not ms_HistoryVisible);
		if ms_HistoryVisible == true then
			Controls.MinimiseHistory:SetText(Locale.Lookup("LOC_BDW_HIDE"));
		else
			Controls.MinimiseHistory:SetText(Locale.Lookup("LOC_BDW_SHOW"));
		end
	end);

	if (player ~= nil) then

		--Controls.MyOfferHistoryStack;

		if(ms_DealHistory[player:GetID()] ~= nil) then

			local turnKeys = {};
			for k in pairs(ms_DealHistory[player:GetID()]) do 
				table.insert(turnKeys, k);
			end
			table.sort(turnKeys, function(a, b) return a > b end);
		
			for _, i in ipairs(turnKeys) do

				Controls.HistoryArea:SetHide(false);

				local deal = ms_DealHistory[player:GetID()][i];

			for ii, item in pairs(deal.Items) do

		local endsInText = "";

			endsInText = (Game:GetCurrentGameTurn() - deal.StartTurn) .. " " .. Locale.Lookup("LOC_BDW_TURNS_AGO");

						local group = CreateHistoryGroup(Controls.OfferHistoryStack, Locale.Lookup("LOC_BDW_PREVIOUS_DEAL") .. " (" .. endsInText .. ")");

						if deal.IsFromLastSession == true then
							group.SessionWarning:SetHide(false);
						end

					PopulateHistoryResources(g_LocalPlayer, group.Left, deal);
					PopulateHistoryResources(g_OtherPlayer, group.Right, deal);

					PopulateHistoryAgreements(g_LocalPlayer, group.Left, deal);
					PopulateHistoryAgreements(g_OtherPlayer, group.Right, deal);
					PopulateHistoryCaptives(g_LocalPlayer, group.Left, deal);
					PopulateHistoryCaptives(g_OtherPlayer, group.Right, deal);
					PopulateHistoryGreatWorks(g_LocalPlayer, group.Left, deal);
					PopulateHistoryGreatWorks(g_OtherPlayer, group.Right, deal);
					PopulateHistoryCities(g_LocalPlayer, group.Left, deal);
					PopulateHistoryCities(g_OtherPlayer, group.Right, deal);
...
-- ===========================================================================
function StoreDealInHistory(otherPlayer : table, deal : table, storeOnlyIfNew : boolean, isFromLastSession : boolean)

	local dealEnactedTurn = 0;
	local localPlayerID = Players[Game.GetLocalPlayer()]:GetID();
	local otherPlayerID = otherPlayer:GetID();

	for item in deal:Items() do
		dealEnactedTurn = item:GetEnactedTurn();
		break
	end

	if dealEnactedTurn <= 0 then
		dealEnactedTurn = Game:GetCurrentGameTurn();
	end

	local transformedDeal = {
		Items = {},
		EndTurn = 0,
		StartTurn = dealEnactedTurn,
		IsFromLastSession = isFromLastSession
	};

	if ms_DealHistory[otherPlayerID] == nil then
		ms_DealHistory[otherPlayerID] = {};
	end

	-- we'll merge multiple deals from the same turn into one - so read existing deal from cache if applicable
	if dealEnactedTurn > 0 and ms_DealHistory[otherPlayerID][dealEnactedTurn] ~= nil then
		if storeOnlyIfNew == true then
			return
		end
		transformedDeal = ms_DealHistory[otherPlayerID][dealEnactedTurn];
	end

	for item in deal:Items() do

		if transformedDeal.EndTurn <= 0 then
			transformedDeal.EndTurn = item:GetEndTurn();
		end

		local endTurn = item:GetEndTurn();

		-- when a new deal is accepted, the item's haven't yet been given an actual end turn, but we can safely calculate it
		if dealEnactedTurn >= Game:GetCurrentGameTurn() and endTurn <= dealEnactedTurn then
			endTurn = transformedDeal.StartTurn + item:GetDuration();
		end

		table.insert(transformedDeal.Items, {
			EndTurn = endTurn,
			ID = item:GetID(),
			Type = item:GetType(),
			...
			ToPlayerID = item:GetToPlayerID(),
			ValueTypeID = item:GetValueTypeID(),
			FromPlayerID = item:GetFromPlayerID(),
			SubTypeNameID = item:GetSubTypeNameID(),
			ValueTypeNameID = item:GetValueTypeNameID()
		});

	end

	ms_DealHistory[otherPlayerID][dealEnactedTurn] = transformedDeal;

end

-- ===========================================================================
function PopulateHistoryResources(player : table, iconList : table, transformedDeal : table)
			...
			if (pDealItem.FromPlayerID == player:GetID()) then
				...
				elseif (DealItemTypes.FAVOR ~= nil and type == DealItemTypes.FAVOR) then
					...
					if (type == DealItemTypes.RESOURCES) then
						...
						local resourceType = pDealItem.ValueType;
						local icon;
						if (iDuration == 0) then
							-- One time
							icon = g_IconOnlyIM:GetInstance(iconList);
...
-- ===========================================================================
function PopulateHistoryBasic(player : table, iconList : table, populateType : number, iconName : string, transformedDeal)
		...
		for i, pDealItem in pairs(transformedDeal.Items) do
			...
			if (pDealItem.FromPlayerID == player:GetID()) then
				...
				if (type == populateType) then
					...
					local typeName = pDealItem.ValueTypeNameID;
					if (typeName ~= nil) then
...
-- ===========================================================================
function PopulateHistoryAgreements(player : table, iconList : table, transformedDeal)
	...
	if (transformedDeal ~= nil) then
		...
		for i, pDealItem in pairs(transformedDeal.Items) do
			...
			if (pDealItem.FromPlayerID == player:GetID()) then
				...
				if (type == DealItemTypes.AGREEMENTS) then
					local icon = g_IconAndTextIM:GetInstance(iconList);
					local info: table = GameInfo.DiplomaticActions[ pDealItem.SubType ];
					if (info ~= nil) then
						SetIconToSize(icon.Icon, "ICON_".. info.DiplomaticActionType, 38);
					end

					icon.AmountText:SetHide(true);
					local subTypeDisplayName = pDealItem.SubTypeNameID;
					if (subTypeDisplayName ~= nil) then
					...
					-- Populate the value pulldown
					if icon ~= nil and pDealItem ~= nil then
						SetHistoryValueText(icon, pDealItem);
					end
...
-- ===========================================================================
function PopulateHistoryGreatWorks(player : table, iconList : table, transformedDeal)
		...
		for i, pDealItem in pairs(transformedDeal.Items) do
			...
			if (pDealItem.FromPlayerID == player:GetID()) then
				...
				if (type == DealItemTypes.GREATWORK) then
					...
					local typeID = pDealItem.ValueTypeID;
					...
					if (typeName ~= nil) then
						icon.IconText:LocalizeAndSetText(typeName);
						strTooltip = Locale.Lookup( GreatWorksSupport_GetBasicTooltip(pDealItem.ValueType, false) );
					else
...
-- ===========================================================================
function PopulateHistoryCaptives(player : table, iconList : table, transformedDeal)

	PopulateHistoryBasic(player, iconList, DealItemTypes.CAPTIVE, "ICON_UNIT_SPY", transformedDeal);

end

-- ===========================================================================
function PopulateHistoryCities(player : table, iconList : table, transformedDeal)
		...
		for i, pDealItem in pairs(transformedDeal.Items) do
			...
			if (pDealItem.FromPlayerID == player:GetID()) then
				...
				if (type == DealItemTypes.CITIES) then
					...
					icon.SelectButton:SetToolTipString( MakeCityToolTip(player, pDealItem.ValueType ) );
				end
			end
		end
...
function OnShow()
	...
	local localPrimaryColor, localSecondaryColor  = UI.GetPlayerColors( g_LocalPlayer:GetID() );
	Controls.MyInventoryGradient:SetColor(DarkenLightenColor(localPrimaryColor,0,245));

	local otherPrimaryColor, otherSecondaryColor  = UI.GetPlayerColors( g_OtherPlayer:GetID() );
	Controls.TheirInventoryGradient:SetColor(DarkenLightenColor(otherPrimaryColor,0,245));

	Controls.MyOfferHistoryGradient:SetColor(DarkenLightenColor(localPrimaryColor,0,100));
	Controls.TheirOfferHistoryGradient:SetColor(DarkenLightenColor(otherPrimaryColor,0,100));

	-- trade history
	PopulatePlayerHistoryPanel(g_OtherPlayer);
	
	if Controls.TradePanel:GetSizeX() < 1560 then
		Controls.HistoryArea:SetAnchor("R,T");
		Controls.HistoryArea:SetOffsetX(35);
		Controls.MinimiseHistory:SetAnchor("L,T");
	end
...
function LateInitialize()
	...
	print("Writing existing deals into history");

	local localPlayer = Players[Game.GetLocalPlayer()];

	local kPlayers          :table = PlayerManager.GetAliveMajors();

	for _, otherPlayer in ipairs(kPlayers) do

		if localPlayer:GetID() ~= otherPlayer:GetID() then

			local pDeals : table = DealManager.GetPlayerDeals(localPlayer:GetID(), otherPlayer:GetID()); 
	
			if (pDeals ~= nil) then
				for i,pDeal in ipairs(pDeals) do
					StoreDealInHistory(otherPlayer, pDeal, true, true);
				end
			end
  ...
  for i, pMinorPlayer in ipairs(PlayerManager.GetAliveMinors()) do
    ...
    if (pMinorPlayerInfluence ~= nil) then 
      ...
      if suzerainID == playerID then
		...
		-- ideally we'd use :GetExportedResourceAmount(), but it always returns 0 now...
          local resourceAmount:number =  pMinorPlayer:GetResources():GetResourceAmount(row.Index);

		--print("Attributes:") for k, v in pairs(getmetatable(pMinorPlayer:GetResources()).__index) do print(k); end; print("End attributes.");
		
          if resourceAmount > 0 then

            local kResource :table = GameInfo.Resources[row.Index];

			if(kResource.ResourceClassType == "RESOURCECLASS_LUXURY") then
...
-- ===========================================================================
function RenderResourceButton(player, entry, resourceCategory, iconList, howAcquired)
	...
	icon.AmountText:SetColor(.8, .8, .8);
	...
	if(howAcquired ~= nil and howAcquired ~= "") then

		local tempstr = Locale.Lookup(resourceDesc.Name).."[NEWLINE][COLOR_RED]";
		icon.SelectButton:SetToolTipString(tempstr..howAcquired);

	end

	if(resourceCategory == 'imported') then

		icon.SelectButton:SetToolTipString(Locale.Lookup(resourceDesc.Name).."[NEWLINE][COLOR_RED]"..Locale.Lookup("LOC_BDW_IMPORTED"));
		icon.SelectButton:SetDisabled(true);
